...
type cachedGatewayForwardingSettings struct {
	...
	rewriteMessageCacheControlUserWhitelist map[int64]struct{}
	...
	if cached, ok := gatewayForwardingCache.Load().(*cachedGatewayForwardingSettings); ok && cached != nil {
		if time.Now().UnixNano() < cached.expiresAt {
			return gatewayForwardingSettingsResult{
				...
				rewriteMessageCacheControlUserWhitelist: cached.rewriteMessageCacheControlUserWhitelist,
	...
	val, _, _ := gatewayForwardingSF.Do("gateway_forwarding", func() (any, error) {
		...
		if err != nil {
			...
			gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
				...
				rewriteMessageCacheControlUserWhitelist: nil,
		...
		gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
			...
			rewriteMessageCacheControlUserWhitelist: cacheRewriteWhitelist,
...
// IsRewriteMessageCacheControlEnabledForUser applies the global switch and
// skips the rewrite for user IDs on the operator exemption list.
func (s *SettingService) IsRewriteMessageCacheControlEnabledForUser(ctx context.Context, userID int64) bool {
	settings := s.getGatewayForwardingSettingsCached(ctx)
	if !settings.rewriteMessageCacheControl {
		return false
	}
	if userID > 0 {
		if _, exempt := settings.rewriteMessageCacheControlUserWhitelist[userID]; exempt {
			return false
		}
	}
	return true
...
// IsCacheModificationUserWhitelisted reports whether a user is exempt from
// operator-controlled cache rewrites and cache billing adjustments.
func (s *SettingService) IsCacheModificationUserWhitelisted(ctx context.Context, userID int64) bool {
	if userID <= 0 {
		return false
	}
	_, ok := s.getGatewayForwardingSettingsCached(ctx).rewriteMessageCacheControlUserWhitelist[userID]
	return ok
}
