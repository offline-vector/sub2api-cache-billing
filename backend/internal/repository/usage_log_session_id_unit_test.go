...
// remains stable immediately before the four fork audit fields, native compaction, and created_at.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 66, "arg-type table must include upstream_request_id, session_id, cache billing audit fields, and native compaction")
	...
	// created_at is last; four cache-billing audit fields and native compaction follow session_id.
	...
	require.Equal(t, []string{"text", "integer", "integer", "numeric", "numeric", "boolean", "timestamptz"},
		usageLogInsertArgTypes[len(usageLogInsertArgTypes)-7:])
}
