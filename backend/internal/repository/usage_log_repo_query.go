...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		UpstreamCacheReadTokens:   upstreamCacheReadTokens,
		CacheBillingRatio:         cacheBillingRatio,
		UpstreamTotalCost:         upstreamTotalCost,
	...
	if turnStateAudit.Valid {
		if err := json.Unmarshal([]byte(turnStateAudit.String), &log.TurnStateAudit); err != nil {
			return nil, fmt.Errorf("decode usage turn state audit: %w", err)
		}
	}
	log.Stream = stream
